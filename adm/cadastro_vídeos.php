<?php	
	...
	$result=null;
	if (($_GET["codigo"]!=null)){
		$sql	= "SELECT codigo,titulo,video FROM titulo where (codigo=0".$_GET["codigo"].")";
...
<form method="post">
		...
		<?php
			...
			if ($_POST['acao']=='excluir'){
				$sql = 'delete FROM titulo where codigo='.$_POST["codigo"];
				//echo $sql;
				mysql_query($sql, $link);
			}
			else if ($_POST ['acao']=='alterar'){
				$sql = "update titulo set titulo='".$_POST[""]."', video='".$_POST["video"]."' where (codigo=".$_POST["codigo"].");";
				//echo $sql;
				mysql_query($sql, $link);
			}
			else if( $_POST['acao']=='inserir'){
				$sql = "insert into titulo (titulo, video) values ('".$_POST["titulo"]."','".$_POST["video"]."');";
				//echo $sql;
				mysql_query($sql, $link);
			}
				else if( $_POST['acao']=='inserir'){
		...
		<?php
			...
			$sql    = 'SELECT codigo,titulo,video FROM titulo order by titulo asc;';
			$result = mysql_query($sql, $link);
			if (!$result) {
